...
class Order(models.Model):
    ...
    def __str__(self):
        return str(f"{self._id} - {self.user.username}")

class OrderItem(models.Model):
    # foreign key to product and order this alows multiple items to be associated with a single order
    product = models.ForeignKey(Product, on_delete=models.SET_NULL, null=True)
    ...
    def __str__(self):
        return str(f"{self._id} - {self.name}")
    
class ShippingAddress(models.Model):
    # one to one means each order has only one shipping address
    ...
    def __str__(self):
        return str(f"{self._id} - {self.address}")